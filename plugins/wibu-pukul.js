let handler = async (m, { conn, usedPrefix, args, command }) => {
      ...
      if (conn.war[m.chat][turn].hp <= 0) {
        ...
        if (turn < 5){
          ...
          if (playerTotal == playerKalah){
            ...
            conn.reply(m.chat, `*TEAM A (ANIME) MENANG KARENA TEAM B (BAIK) TERLALU BAIK SAMPAI GOBLOK SEMUA*\n\n*TEAM A :*\n` + teamA.map((v, i )=> `${conn.war[m.chat][i].hp > 0 ? '❤️ ' : '🐦️ ' }@${v.split('@')[0]} (+ Rp. ${Number(conn.war2[m.chat].money).toLocaleString()})`).join`\n` + "\n\n*TEAM B :*\n" + teamB.map((v, i) => `${conn.war[m.chat][i+5].hp > 0 ? '❤️ ' : '🐦 ' }@${v.split('@')[0]} (- Rp. ${Number(conn.war2[m.chat].money).toLocaleString()})`).join`\n`,m, {contextInfo: {
              mentionedJid: teamAB
            }})
            delete conn.war[m.chat]
            delete conn.war2[m.chat]
  ...
  if (!conn.war2[m.chat].war) return m.reply(`*Permainan belom dimulai, ketik ".wibu mulai" untuk memulai pertarungan.*`)
  for (let i=0;i<10;i++){
    if (m.sender == conn.war[m.chat][i].user){
      if (i != conn.war2[m.chat].turn) {
        conn.reply(m.chat,`*Sekarang adalah giliran wibu @${conn.war[m.chat][conn.war2[m.chat].turn].user.split('@')[0]} untuk memukul.* 🐦`,m, {contextInfo : { mentionedJid : [conn.war[m.chat][conn.war2[m.chat].turn].user]}})
        cekAFK(conn.war2[m.chat].turn)
      }
    }
  }
  if (!args[0]) return m.reply(`*Tag wibu yang akan di pukul*\n*Ketik .wibu pemain*`)
       ...
       let  target = args[0]
        ...
        for (let i=0;i<10;i++){
          if (conn.war[m.chat][i].user == target){
            ...
            conn.reply(m.chat,`*@${attacker.split('@')[0]} memukul @${target.split('@')[0]} sampai nyawanya berkurang ${pointAttacker * 500} (Sisa HP: ${conn.war[m.chat][i].hp})*\n\n*@${attacker.split('@')[0]} [${pointAttacker*10}%] - [${pointTarget*10}%] @${target.split('@')[0]}*\n*Level wibu sangat mempengaruhi keberhasilan.*`,m,{contextInfo : {mentionedJid : [attacker, target]}})
            await sleep(2000)
            if (conn.war[m.chat][i].hp <= 0) conn.reply(m.chat,`Wibu *@${target.split(`@`)[0]} sudah mati dalam permainan karna kamu beban.*`,m, {contextInfo : {mentionedJid : [target]}})
            success = true
          }
        }
      }
  ...
  if (conn.war2[m.chat].turn < 5){
    ...
    if(userAktif == userMati){
      ...
      conn.reply(m.chat, `*TEAM A (ANIME) MENANG KARENA TEAM B (BAIK) TERLALU BAIK SAMPE GOBLOK SEMUA*\n\n*TEAM A :*\n` + teamA.map((v, i )=> `${conn.war[m.chat][i].hp > 0 ? '❤️ ' : '🐦 ' }@${v.split('@')[0]} (+ Rp. ${Number(conn.war2[m.chat].money).toLocaleString()})`).join`\n` + "\n\n*TEAM B :*\n" + teamB.map((v, i) => `${conn.war[m.chat][i+5].hp > 0 ? '❤️ ' : '🐦️ ' }@${v.split('@')[0]} (- Rp. ${Number(conn.war2[m.chat].money).toLocaleString()})`).join`\n`,m, {contextInfo: {
        mentionedJid: teamAB
      }})
      delete conn.war[m.chat]
      delete conn.war2[m.chat]
    ...
    await sleep(2000)
    conn.reply(m.chat,`*Giliran @${conn.war[m.chat][conn.war2[m.chat].turn].user.split('@')[0]} untuk memukul (Waktu 90 detik)*\n\n.wibu pemain = statistik pemain\n.pukul @tag = serang wibu🐦`,m, {contextInfo : {mentionedJid: [conn.war[m.chat][conn.war2[m.chat].turn].user]}})
    cekAFK(conn.war2[m.chat].turn)
  }
...
handler.help = ['pukul','serang']
handler.tags = ['game']
handler.command = /^(pukul|serang)$/i
handler.group = true
