...
if (!effect) throw '*Contoh penggunaan*\n.logo space Rezdev\n\n[ *_LIST LOGO MAKER_* ]\n\n • .logo ' + effects.map(v => v.title).join('\n• #logo ')
effect = effect.toLowerCase()
if (!effects.find(v => (new RegExp(v.title, 'gi')).test(effect))) throw `effeck?`
let text = txt.replace(new RegExp(effect, 'gi'), '').trimStart()
if (text.includes(split)) text = text.split(split)
text = Array.isArray(text) ? text : [text]
let res = await textpro(effect, ...text)
if (typeof res == 'number') throw res == -1 ? `efeck?` : `*Contoh penggunaan* ${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`
let result = await axios.get(res, {
responseType: 'arraybuffer'
})
await conn.sendFile(m.chat, result.data, 'Error.jpg', `Nih kak logo ${effect} nya`, m)
}
handler.help = ['logo']
handler.tags = ['main']
handler.command = /^(logo|maker)$/i
