...
let handler = async (m, { conn, usedPrefix: _p }) => {
  try {
    ...
    let pp = 'https://telegra.ph/file/09cb67b8e43be35e072a0.jpg'
  ...
  // conn.sendButton(m.chat, `*${wish()}, ${name} 👋*`, text.trim(), await genProfile(conn, m), [['Speedtest', _p + 'ping'], ['Owner', _p + 'owner']],m, { viewOnce: true })
   conn.send2ButtonLoc(m.chat, await conn.resize(pp, 280, 210), text.trim(), `Hai kak @${m.sender.split`@`[0]} 🍟\nSaya bot whatsapp yang di buat oleh © Rezdev🐋, Jika terdapat fitur error pada bot segera laporkan kepada owner ya kak>~<`, 'Owner', '.owner', 'Sewabot', '.sewa', m)
